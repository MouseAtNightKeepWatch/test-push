<template>
	<div class="drawer" :class="{'left-side': !rightSide, 'rightSide': rightSide}">
		<div 
			class="drawer-opener touch-only mobile-only"
			v-touch:pan="openByTouch"
			v-touch-options:pan="{direction: 'horizontal'}"
			:class="{'fixed-left': !rightSide, 'fixed-right': rightSide}">&nbsp
		</div>
		<div 
			ref="backdrop"
    ...
    export default {
        ...
        methods: {
            setState(state, done) {
                ...
                let fn = theme.current === 'oth' ? this.__iosToggleAnimate : this.__matToggleAnimate
                fn(this.opened ? 0.01 : 1, done)
            },
            __matToggleAnimate(percentage, done) {
                const node = this.$refs.content
                const backdrop = this.$refs.backdrop
                const currentPosition = getCurrentPosition(node)
                const closePosition = (this.rightSide ? 1 : -1) * this.width
                Velocity(node, 'stop')
                Velocity(node,
            ...
            __iosToggleAnimate(percentage, done) {
                ...
                let currentPosition = getCurrentPosition(appContainer)
                let openPosition = (this.rightSide ? -1 : 1) * this.width
            ...
            openByTouch(event) {
                ...
                let position = Math.abs(event.deltaX)
                let target
                let fn
                let percentage
            ...
            closeByTouch(event) {
                ...
                let target, fn, percentage, position, initialPosition
        ...
        mounted() {
            const toggles = this.$el.closest('.layout').getElementsByClassName((this.rightSide ? 'right' : 'left') + '-drawer-opener')
            this.width = Utils.dom.width(this.$refs.content)
            Array.prototype.slice.call(toggles).forEach(el => {
                el.addEventListener('click', () => {
                    this.setState(true)
                })
            })

            Array.prototype.slice.call(this.$refs.content.getElementsByClassName('drawer-closer')).forEach(el => {
                el.addEventListener('click', () => {
                    this.setState(false)
                })
            })
